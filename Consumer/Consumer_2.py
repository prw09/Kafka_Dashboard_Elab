...
def consume_messages():
    ...
    consumer = KafkaConsumer(
        "dbo.LogException", "dbo.Machine","dbo.AppVersionLog",
        bootstrap_servers=KAFKA_BOOTSTRAP,
        value_deserializer=lambda x: json.loads(x.decode('utf-8')),
        #api_version=(2, 7),
        enable_auto_commit=False,
...
def process_message(cursor, message):
    ...
    try:
        # ==================================================
        # SPECIAL LOGIC FOR dbo.AppVersionLog
        # ==================================================
        if table_name == "dbo.AppVersionLog":

            #  Process ONLY when IsSync = 0
            if record.get("IsSync") != 0:
                return True

            cursor.execute(
                """
                SELECT 1 FROM dbo.AppVersionLog
            ...
            if cursor.fetchone():
                ...
                query = """
                        INSERT INTO dbo.AppVersionLog (
                            ...
                            InstallationSystemName, UserName,
                            InstrumentName, LocationName,
                            CenterId, LogDate,
                            BuildVersion, BuildDate, IsSync
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                    """
                params = (
                    record["Id"],
        ...
        logger.error(f"AppVersionLog error: {e}")
        return False
    #IF not appversionlog table  other tables
