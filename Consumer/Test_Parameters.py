...
def consume():
    ...
    # Parse broker list — supports both single and multi-broker from env
    ...
    consumer = KafkaConsumer(
        TABLE_NAME,
        bootstrap_servers=broker_list,        # ✅ Now passes both 9095 + 9096
        ...
        request_timeout_ms=40000,             # ✅ Added: must be > session_timeout_ms

        auto_offset_reset="latest",

        # ✅ Reconnect settings for VPN/external broker stability
        ...
        try:
            for _, messages in batch.items():
                for msg in messages:
                    ...
                    # ✅ FIX: use safe_process_batch consistently (not raw process_batch)
                    if len(all_records) >= BATCH_SIZE:
                        ...
                        if success:
                            ...
                            logger.warning(f"⚠️ Mid-batch partially failed: {len(all_records)}")
                        all_records.clear()
            ...
            if all_records:
                ...
                if success:
                    ...
                    logger.warning(f"⚠️ Final batch partially failed: {len(all_records)}")
                all_records.clear()
